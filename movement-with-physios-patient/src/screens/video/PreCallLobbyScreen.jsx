 ...
 * PreCallLobbyScreen
 *
 * Shown before a therapist joins a video session. Loads the call from
 * the backend, displays the therapist's name + scheduled time, starts
 * a local camera preview (this is where iOS first prompts for mic +
 * camera permission — NOT in VideoCallScreen), and gates the
 * "Join Call" CTA behind the server-computed `canJoin` flag.
 *
 * Route params:
 *   { callId: string, bookingId?: string }
 *
 * On Join: navigation.replace to PATIENT_ROUTES.VIDEO_CALL with
 *   { callId, otherUserId, assessmentId, bookingId }.
 *   assessmentId is passed for route-param parity with the therapist
 *   app but VideoCallScreen ignores it (patient never sees the panel).
 *
 * Ported from movement-with-physios/apps/therapist/src/screens/video/PreCallLobbyScreen.jsx
 * with patient-specific simplifications:
 *   - No body-part badge (patient does not see body part).
 *   - No getBooking call — only getCall is needed.
 *   - "Therapist" fallback for otherParty.name (was "Patient").
 */
...
export default function PreCallLobbyScreen({ navigation, route }) {
  ...
  // ── Load call on mount, then poll every 5s to pick up canJoin flipping
  //    true as the join window opens.
  useEffect(function () {
    var cancelled = false;
    async function load() {
      var callResp = await getCall(callId);
      if (cancelled) return;
  ...
  // ── Local camera preview (no peer connection here). This is where iOS
  //    shows the mic/camera permission dialog the first time. We want
  //    that to happen here in the lobby — NOT after the user has hit
  //    "Join Call" and is staring at a black VideoCallScreen.
  useEffect(function () {
    var cancelled = false;
  ...
  function handleJoin() {
    ...
    navigation.replace(PATIENT_ROUTES.VIDEO_CALL, {
      callId: callId,
  ...
  var otherName = (call && call.otherParty && call.otherParty.name) || 'Therapist';
  var scheduledAt = call && call.scheduledAt;
...
var styles = StyleSheet.create({
  ...
  joinBtnText: { color: '#FFFFFF', fontSize: fonts.md, fontWeight: fonts.semibold },

  centered: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 24, gap: 10 },
  errorText: { color: colors.danger, fontSize: fonts.sm, textAlign: 'center' },
  secondaryBtn: {
