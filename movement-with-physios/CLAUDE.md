...
- **WebRTC:** `react-native-webrtc@124.0.6` + `@config-plugins/react-native-webrtc@13.0.0` (Phase 1 — natives linked, no UI yet)
- **Dev client:** `expo-dev-client@~6.0.21` + `expo-build-properties@~1.0.10` (SDK 54 versions — NOT ^55.x)

## Dev client + WebRTC plumbing (Phase 1, no UI yet)

Expo Go no longer works — app links `react-native-webrtc` natively. Use
the custom dev-client (`npx expo run:ios --device` once, then
`npx expo start --dev-client`).

- **Version pin matters.** `npm install expo-dev-client` grabs `^55.x`
  which crashes the iOS build (`AppContext.reloadAppAsync()` doesn't
  exist in SDK 54 expo-modules-core). Always install via
  `npx expo install` so SDK-matched versions land.
- **`metro.config.js` exports bypass.** `react-native-webrtc` imports
  `event-target-shim/index`; v6's `exports` field only exposes `.`,
  so Metro's Node-exports check blocks the subpath even with
  `resolveFrom`. Fix resolves directly to the literal `index.js`
  inside the nested v6 under `react-native-webrtc`. Existing Clerk
  `react-dom` stub is preserved alongside.
- **app.json natives.** iOS `UIBackgroundModes: ["audio"]`, iOS
  deploymentTarget `15.1`; Android `minSdkVersion 24` / `compileSdkVersion 34`
  + perms (`CAMERA, RECORD_AUDIO, MODIFY_AUDIO_SETTINGS, INTERNET,
  ACCESS_NETWORK_STATE, WAKE_LOCK`). Mic/camera permission prompt
  strings live on the `@config-plugins/react-native-webrtc` plugin
  entry. Existing `NSPhotoLibraryUsageDescription` and `expo-image-picker`
  plugin remain.
- **`.npmrc` with `legacy-peer-deps=true`** committed at app root for
  `@clerk/clerk-expo 2.x` peer-dep resolution.
- **`ios/` regenerated by prebuild** — Info.plist, Podfile.lock,
  project.pbxproj all touched. Continue tracking selectively per the
  existing pattern (Pods/ etc. filtered by `ios/.gitignore`).
- **One-time environment**: needs Xcode ≥ 26.5, iOS device on iOS
  ≤ Xcode's bundled SDK, Developer Mode enabled on the iPhone, Personal
  Team set in Xcode Signing & Capabilities.
